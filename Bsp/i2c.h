...
#define DIRECT_LSB 0x00
#define DIRECT_MSB 0x01
...
u8 iic_Command(u8 deviceAddr,u8 cmd);  //发送命令，无写入
uchar getTest(void);  //获取读取标志位
void resetTest(void);		//复位读取标志位
void iic_burst_read(uchar* array,uchar length,uchar deviceAddr,uchar regAddr,u8 direct);
#endif
